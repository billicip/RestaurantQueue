...
# Configuration
NUM_SEEDS = 100
VIP_BASE_SEED = 1000   # same fixed seeds as Stage 1 for fair comparison

SETTINGS = [
    ("FCFS",       os.path.join(BASE_DIR, "settings", "data_driven_70_fcfs.json")),
    ("group_size", os.path.join(BASE_DIR, "settings", "data_driven_70_group_size.json")),
    ("priority",   os.path.join(BASE_DIR, "settings", "data_driven_70_priority.json")),
]
...
DETAILED_CSV = os.path.join(
    RESULTS_DIR, "stage4_data_driven_queue_comparison_detailed.csv")
SUMMARY_CSV = os.path.join(
    RESULTS_DIR, "stage4_data_driven_queue_comparison_summary.csv")
...
def check_files():
    ...
    for label, path in SETTINGS:
        if not os.path.exists(path):
            missing.append(path)
    if missing:
        print("ERROR: Data-driven settings files not found:")
        for p in missing:
            print(f"  {p}")
        sys.exit("Please run: python generate_data_driven_config.py")
...
def main():
    print("=" * 70)
    print("  Stage 4: Queue Discipline Comparison — Data-Driven 70-seat Tables")
    print("=" * 70)
    ...
    # Print results table
    print("\n" + "=" * 70)
    print("  Stage 4 Mean Results (across 100 lunch datasets)")
    print("=" * 70)
